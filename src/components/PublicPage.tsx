    ...
    <div className="mx-auto max-w-7xl space-y-8">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
        <div className="bg-slate-900 px-6 py-8 text-white md:px-8">
          ...
          <p className="mt-4 max-w-3xl text-sm leading-relaxed text-blue-100 md:text-base">{description}</p>
        </div>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {routes.map((route) => (
        <Link
          key={route.href}
          href={route.href}
          className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition hover:-translate-y-1 hover:border-blue-300"
        >
